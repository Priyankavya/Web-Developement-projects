...
class Product{
    async getProduct(){
        ...
        products=products.map(item=>
        {
            ...
            const(id)=item.sys;
            const image=item.fields.imagge.fields.file.url;
    ...
    addBtn.forEach((btn)=>
    {
        btn.addEventListener("click",(event)=>
        {
            Cart.map((item)=>
            {
                ...
                Storage.saveCart(Cart)
                this.setCartValues(Cart)
                const ammountUi=event.currentTarget.parentElement.children[1]
                ammountUi.innerHTMLitem.amount
                }
            })
        })
    })
            ...
            Cart.map((item)=>{
                if(item.id===id)
                {
                    ...
                    if(item.amount>0)
                    {
                        ...
                        this.setCartValues(Cart)
                        const ammountUi=event.currentTarget.parentElement.children[1]
                        ammountUi.innerHTML=item.amount
                    }
